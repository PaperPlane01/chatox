...
export class MessageRepository extends AbstractRepository<MessageEntity, MessageRelationships> {
	...
	async restoreEntityPatch(id: string): Promise<EntitiesPatch> {
		...
		const relationships = await this.loadRelationships(message);
	...
	async restoreEntityPatchForEntities(entities: MessageEntity[]): Promise<EntitiesPatch> {
		...
		const {users, chatRoles, uploads, messages: referredMessages, stickers} = await this.loadRelationshipsForArray(entities);
	...
	async loadRelationships(entity: MessageEntity): Promise<MessageRelationships> {
		...
		const [users, chatRole, uploads, referredMessage, sticker] = await Promise.all([
			...
			isDefined(referredMessageId) ? this.findById(referredMessageId) : undefined,
	...
	async loadRelationshipsForArray(messages: MessageEntity[]): Promise<MessageRelationships> {
		...
		const [users, chatRoles, uploads, referredMessages, stickers] = await Promise.all([
			...
			this.repositories.getRepository("uploads")?.findAllById(uploadsIds) ?? emptyArray<Upload<any>>(),
			this.findAllById(referredMessagesIds),
