...
@Service
@Transactional
class ChatBlockingServiceImpl(
    private val chatBlockingRepository: ChatBlockingMongoRepository,
    ...
    override fun blockUser(
        ...
        return mono {
            ...
            val deleteMessagesSince =
                createChatBlockingRequest.deleteMessagesSince ?: ZonedDateTime.now().minusMinutes(5L)
            var deletedMessages = messageRepository.findBySenderIdAndCreatedAtAfter(
                ...
                .collectList()
                .awaitFirst()
            deletedMessages = deletedMessages.map {
                it.copy(
                    deleted = true,
    ...
    override fun getActiveBlockingsByChat(
        chatId: String,
        paginationRequest: PaginationRequest
    ): Flux<ChatBlockingResponse> {
        val localUserCache = HashMap<String, UserResponse>()

        return findChatById(chatId)
            .flatMapMany { chat ->
                chatBlockingRepository.findByChatIdAndBlockedUntilAfterAndCanceled(
                    chatId = chat.id,
                    canceled = false,
    ...
    override fun getNonActiveBlockingsByChat(
        chatId: String,
        paginationRequest: PaginationRequest
    ): Flux<ChatBlockingResponse> {
        val localUserCache = HashMap<String, UserResponse>()
        ...
        return findChatById(chatId)
            .flatMapMany { chat ->
                chatBlockingRepository.findByChatIdAndBlockedUntilBeforeOrCanceled(
                    chatId = chat.id,
                    canceled = true,
                    date = ZonedDateTime.now(),
                    pageable = paginationRequest.toPageRequest()
    ...
    override fun getAllBlockingsByChat(
        chatId: String,
        paginationRequest: PaginationRequest
    ): Flux<ChatBlockingResponse> {
        val localUserCache = HashMap<String, UserResponse>()
